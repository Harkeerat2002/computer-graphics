...
public:
	...
	Hit intersect(Ray ray)
	{
		...
		if (D <= radius)
		{
			float t1 = glm::dot(center, ray.direction) + sqrt(radius * radius - glm::pow(D, 2));
			float t2 = glm::dot(center, ray.direction) - sqrt(radius * radius - glm::pow(D, 2));
			float closest = glm::min(t1, t2);
			if (closest >= 0)
			{
				hit.hit = true;
				hit.intersection = ray.direction * closest;
