...
public:
	...
	Hit intersect(Ray ray){
		...
		if(t<1e-3 || hit.intersection.y>1 || hit.intersection.y<0){
			t = t2;
			hit.intersection = o + t*d;
			if(t<1e-3 || hit.intersection.y>1 || hit.intersection.y<0){
				return hit;
...
// Assignment 6 declaratin of trace ray to use it in the PhongModel
glm::vec3 trace_ray(Ray ray, bool outside, int iterations = 0);
...
*/
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material, bool outside){
	...
	for(int light_num = 0; light_num < lights.size(); light_num++){
		...
		// Assignment 6 adding shadows
		Hit closest_hit;
		closest_hit.hit = false;
		closest_hit.distance = INFINITY;

		for (int i = 0; i < objects.size(); i++){
			Hit hit = objects[i]->intersect(Ray(point + light_direction * 0.001f, light_direction));
			if (hit.hit == true && hit.distance < r){
				closest_hit = hit;
			}
		}

		if (closest_hit.hit == false || NdotL < 0){
			color += lights[light_num]->color * (diffuse + specular) / r / r;
		} else {
			color *= lights[light_num]->color;
		}
		
		// Assignment 6 adding reflections
		if (material.reflection == 1.0) {

			glm::vec3 reflec = glm::normalize(glm::reflect(-view_direction,normal));
			Ray reflected_ray(point + reflec * 0.001f, reflec);

			color = material.reflection*trace_ray(reflected_ray, outside) + (1-material.reflection)*color;
		}

		// Assignment 6 adding reflections
		// if (material.refraction == 2.0) {

		// 	float beta = material.refraction;
		// 	glm::vec3 refrac = glm::normalize(glm::refract(view_direction, normal, beta));

		// 	if (!outside){
		// 		float beta = 1.0f / material.refraction;
		// 		glm::vec3 refrac = glm::normalize(glm::refract(-view_direction, -normal, beta));
		// 	}

		// 	color += material.refraction * trace_ray(Ray(closest_hit.intersection + refrac * 0.01f, refrac), !outside);

		// 	// color += trace_ray(Ray(closest_hit.intersection + refrac * 0.01f, refrac), outside);
 ...
 */
glm::vec3 trace_ray(Ray ray, bool outside, int iterations){
	...
	if(closest_hit.hit){
		...
		if(closest_hit.object->getMaterial().refraction >= 1.0){
			float beta = 1.0f / closest_hit.object->getMaterial().refraction;
			glm::vec3 refrac = glm::normalize(glm::refract(ray.direction, closest_hit.normal, beta));

			if (!outside){
				beta = closest_hit.object->getMaterial().refraction / 1.0f;
				refrac = glm::normalize(glm::refract(ray.direction, -closest_hit.normal, beta));
			}
			// std::cout << outside<<"\n";
			iterations += 1;
			color += closest_hit.object->getMaterial().refraction * trace_ray(Ray(closest_hit.intersection, refrac), !outside, iterations);
		} else {
			color = PhongModel(closest_hit.intersection, closest_hit.normal, closest_hit.uv, glm::normalize(-ray.direction), closest_hit.object->getMaterial(), outside);
		}
...
void sceneDefinition (){
	...
	// Assignment 6 
	Material reflection_specular;
	reflection_specular.ambient = glm::vec3(0.0f, 0.0f, 0.0f);
	reflection_specular.diffuse = glm::vec3(0.0f, 0.0f, 0.0f);
	reflection_specular.specular = glm::vec3(0.0);
	reflection_specular.shininess = 0.0;
	reflection_specular.reflection = 1.0;

	Material refraction_specular;
	refraction_specular.ambient = glm::vec3(0.0f, 0.0f, 0.0f);
	refraction_specular.diffuse = glm::vec3(0.0f, 0.0f, 0.0f);
	refraction_specular.specular = glm::vec3(0.0);
	refraction_specular.shininess = 0.0;
	refraction_specular.refraction = 2.0;
	refraction_specular.reflection = 0.0;


	objects.push_back(new Sphere(1.0, glm::vec3(1,-2,8), reflection_specular));
	objects.push_back(new Sphere(0.5, glm::vec3(-1,-2.5,6), red_specular));
	objects.push_back(new Sphere(2.0, glm::vec3(-3,-1,8), refraction_specular));
...
int main(int argc, const char * argv[]) {
    ...
    for(int i = 0; i < width ; i++)
        for(int j = 0; j < height ; j++){
			...
			image.setPixel(i, j, toneMapping(trace_ray(ray, true)));
