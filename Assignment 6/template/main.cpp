/**
@file main.cpp
@author Mak Fazlic & Harkeerat Singh Sawhney
*/
...
vec3 trace_ray(Ray ray, int in_medium, int recursion_depth);
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material)
{
	...
	for (int light_num = 0; light_num < lights.size(); light_num++)
	{
		...
		Ray to_light = Ray(point + light_direction * 0.001f, light_direction);
		int did_hit = 1;

		for (int i = 0; i < objects.size(); i++)
		{
			Hit hit = objects[i]->intersect(to_light);
		...
		color += lights[light_num]->color * (diffuse + specular) / r / r * vec3(did_hit);

		if (material.reflection)
		{
			vec3 reflection = reflect(-view_direction, normal);
			Ray light_reflec(point + reflection * 0.001f, reflection);
			color = trace_ray(light_reflec, 0, 0);
		}
	}
 ...
 */
glm::vec3 trace_ray(Ray ray, int in_medium, int recursion_depth)
{
	int MAXIMAL_RECURSION_DEPTH = 100;
	float AIR_REFRACTION = 1.0f;
	...
	float object_refraction = closest_hit.object->getMaterial().refraction;
	// cout << object_refraction;

	if (recursion_depth >= MAXIMAL_RECURSION_DEPTH || !closest_hit.hit)
	{
		// cout << "1" << endl;
		return vec3(0.0);
	}
	if (object_refraction >= AIR_REFRACTION)
	{
		// cout << "2" << endl;
		// cout << "3" << endl;
		recursion_depth++;
		...
		vec3 i = closest_hit.intersection;

		if (in_medium == 1)
		{
			in_medium = 0;
			//cout << in_medium << endl;
			refraction_coefficient = object_refraction / AIR_REFRACTION;
			refraction_vector = normalize(refract(rd, -n, refraction_coefficient));
		...
		else
		{
			in_medium = 1;
			//cout << in_medium << endl;
			refraction_coefficient = AIR_REFRACTION / object_refraction;		
			refraction_vector = normalize(refract(rd, n, refraction_coefficient));
		}
		Ray ray_from_intersect = Ray(i + refraction_vector * 0.001f, refraction_vector);
		
		final_color += object_refraction * trace_ray(ray_from_intersect, in_medium, recursion_depth);
	}
	else
	{
		final_color = PhongModel(closest_hit.intersection, closest_hit.normal, closest_hit.uv, glm::normalize(-ray.direction), closest_hit.object->getMaterial());
...
void sceneDefinition()
{
	...
	Material blue_reflection;
	blue_reflection.ambient = glm::vec3(0.0f, 0.0f, 0.0f);
	blue_reflection.diffuse = glm::vec3(0.0f, 0.0f, 0.0f);
	blue_reflection.specular = glm::vec3(0.00);
	blue_reflection.shininess = 0.0;
	blue_reflection.reflection = true;

	Material sphere_refraction;
	sphere_refraction.ambient = glm::vec3(0.0f, 0.0f, 0.0f);
	sphere_refraction.diffuse = glm::vec3(0.0f, 0.0f, 0.0f);
	sphere_refraction.specular = glm::vec3(0.00);
	sphere_refraction.shininess = 0.0;
	sphere_refraction.refraction = 2.0;

	objects.push_back(new Sphere(2.0, glm::vec3(-3, -1, 8), sphere_refraction));
	objects.push_back(new Sphere(1.0, glm::vec3(1, -2, 8), blue_reflection));
...
int main(int argc, const char *argv[])
{
	...
	for (int i = 0; i < width; i++)
		for (int j = 0; j < height; j++)
		{
			...
			image.setPixel(i, j, toneMapping(trace_ray(ray, 0, 0)));
		}
