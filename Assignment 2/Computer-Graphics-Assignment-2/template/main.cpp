...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec3 view_direction, Material material)
{
	...
	vec3 my_point = point;
	vec3 my_view = view_direction;
	Material my_material = material;
	glm::vec3 distance_attenuation;
	...
	*/
	for (int i = 0; i < lights.size(); i++)
	{
		...
		vec3 light = normalize(light_position - my_point);
		vec3 rev_light = (2 * dot(my_normal, light) * my_normal) - light;
		if (dot(my_normal, light) > 0)
		{
			color += my_material.diffuse * dot(my_normal, light) * light_color;
		}
		else
		{
			color += vec3(0.0);
		...
		if (dot(rev_light, my_view) > 0)
		{
			color += (my_material.specular * pow(dot(my_view, rev_light), my_material.shininess) * light_color);
		}
		else
		{
			color += vec3(0.0);
		...
		distance_attenuation = glm::vec3(1)/(glm::pow(light, glm::vec3(i+1)));
	}
	color += my_material.ambient * ambient_light * distance_attenuation;
...
void sceneDefinition()
{
	...
	*/
	...
	objects.push_back(new Sphere(0.5, glm::vec3(-1, -2.5, 6), red_specular));
	objects.push_back(new Sphere(1.0, glm::vec3(3, -2, 6), green_specular));

	//lights.push_back(new Light(glm::vec3(1, 2, 2), glm::vec3(0.4)));
	lights.push_back(new Light(glm::vec3(1, 26.0, 5.0), glm::vec3(0.4)));
